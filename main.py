...
data_loader_tra, data_loader_val, data_loader_tst, vocab, program_number, atomic_relation_num = prepare_data_seq(batch_size=config.batch_size)
...
try:
    ...
    for n_iter in tqdm(range(1000000)):
        if config.gradient_accumulation_steps>1:
            if config.multitask:
                loss, ppl, kld, bow, elbo = model.train_n_batch(
                    [next(data_iter) for i in range(config.gradient_accumulation_steps)], n_iter)
            else:
                loss, ppl, kld, bow, elbo, _, _, _ = model.train_n_batch([next(data_iter) for i in range(config.gradient_accumulation_steps)],n_iter)
        else:
            if config.multitask:
                loss, ppl, kld, bow, elbo, _, _, _ = model.train_one_batch(next(data_iter), n_iter)
            else:
