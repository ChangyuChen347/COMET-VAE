...
class DecoderLayer_2(nn.Module):
    """
    Represents one Decoder layer of the Transformer Decoder
    Refer Fig. 1 in https://arxiv.org/pdf/1706.03762.pdf
    NOTE: The layer normalization step has been moved to the input as per latest version of T2T
    ...
    def __init__(self, hidden_size, total_key_depth, total_value_depth, filter_size, num_heads,
        ...
        super(DecoderLayer_2, self).__init__()
        ...
        self.multi_head_attention_enc_dec_c = MultiHeadAttention(hidden_size, total_key_depth, total_value_depth,
                                                                 hidden_size, num_heads, None, attention_dropout)

        self.multi_head_attention_enc_dec_r = MultiHeadAttention(hidden_size, total_key_depth, total_value_depth,
                                                                 hidden_size, num_heads, None, attention_dropout)
        ...
        self.layer_norm_mha_enc = LayerNorm(hidden_size)
        self.layer_norm_ffn = LayerNorm(hidden_size)
        self.layer_norm_ctx = LayerNorm(hidden_size)
        self.layer_norm_c = LayerNorm(hidden_size)
    ...
    def forward(self, inputs):
        ...
        x, encoder_outputs, attention_weight, mask = inputs
        mask_src, mask_src_c, mask_src_r, dec_mask = mask
        ...
        x_norm = self.layer_norm_mha_enc(x)
        # Multi-head encoder-decoder attention
        y, attention_weight = self.multi_head_attention_enc_dec(x_norm, encoder_outputs[0], encoder_outputs[0], mask_src)
        #x = self.dropout(x + y)
        #x_norm = self.layer_norm_ctx(x)
        y_c, _ = self.multi_head_attention_enc_dec_c(x_norm, encoder_outputs[1], encoder_outputs[1], mask_src_c)
        # Dropout and residual after encoder-decoder attention
        # x = self.dropout(x + y_c)
        #x_norm = self.layer_norm_c(x)
        y_r, _ = self.multi_head_attention_enc_dec_r(x_norm, encoder_outputs[2], encoder_outputs[2], mask_src_r)
        #x = self.dropout(x + y_r)
        tmp = x + y
...
import numpy as np
from src.test import count_new_metrics
from emotion_classifier.classify_emotion import cls_emo
def evaluate(model, data,  ty='valid', max_dec_step=50,  do_label=False, n_iter=0):
    model.eval()
   
    
    res_store_file = config.test_name + str(n_iter) + '.' + 'res.pkl'

   
    post_label_file = config.test_name + '.' + ty + '.post.pkl'
    post_c_r_label_file = config.test_name + '.' + ty + '.c_r_post.pkl'
    ...
    bow = []
    elbo = []
    k_r = [[] for _ in range(10)]
    k_p = [[] for _ in range(10)]
    k_r_d = [[] for _ in range(10)]
    k_p_d = [[] for _ in range(10)]
    alphas = []
    post_c_r_labels = []
    post_labels = []
    bow_res = []
    all_batch_npz = []
    for j, batch in enumerate(data):
        return_dict = model.train_one_batch(batch, 0, train=False)
        post_label = return_dict['encoding']
        batch_vq_label = post_label.detach().cpu().numpy().tolist()
        batch_vq_label = [r.index(1) for r in batch_vq_label]
        post_labels.extend(batch_vq_label)
        loss = return_dict['loss_rec']
        ppl = return_dict['ppl']
        dis_loss = return_dict['dis_loss']
        bow_prog = return_dict['aux']
        elbo_prog = return_dict['elbo']
        batch_npz = return_dict['z']
        all_batch_npz.append(batch_npz) #[[bs, 10, dim]]
        if config.multitask:
            eacc = return_dict['emo_acc']
            eloss = return_dict['emo_loss']
            acc.append(eacc)
            el.append(eloss)
        l.append(loss)
        p.append(ppl)
        ...
        #     if j > 50:
        #         break
        # k_rf = batch['k_target_txt']  # bs, k_num, seq_w
        # k_rf_set = batch['k_target_txt_set']
        # k_pred = return_dict['bow']  # bs, k_num, seq_w [['w1','w2'], ['w3']]
        # for i, k_seq in enumerate(k_pred):
        #     for k_index in range(10):
        #         pred_seq = k_seq[k_index]
        #         tgt_seq = k_rf[i][k_index]
        #         tgt_seq_d = k_rf_set[i][k_index]
        #         pred_seq = pred_seq[:20]
        #         if i == 0 and j == 0:
        #             print(pred_seq)
        #             print(tgt_seq)
        #             print(tgt_seq_d)
        #         pred_seq_set = set(pred_seq)
        #         tgt_seq_set = set(tgt_seq)
        #         tgt_seq_d_set = set(tgt_seq_d)
        #         prec_num = 0
        #         recall_num = 0
        #         for t in tgt_seq:
        #             if t in pred_seq_set:
        #                 recall_num += 1
        #         for t in pred_seq:
        #             if t in tgt_seq_set:
        #                 prec_num += 1
        #         recall = recall_num / len(tgt_seq)
        #         precision = prec_num / 20
        #         k_r[k_index].append(recall)
        #         k_p[k_index].append(precision)
        #
        #         prec_num = 0
        #         recall_num = 0
        #         for t in tgt_seq_d:
        ...
        #                 recall_num += 1
        #         for t in pred_seq:
        #             if t in tgt_seq_d_set:
        #                 prec_num += 1
        #         recall = recall_num / len(tgt_seq_d)
        #         precision = prec_num / 20
        #         k_r_d[k_index].append(recall)
        #         k_p_d[k_index].append(precision)


        #pbar.set_description("loss:{:.4f} ppl:{:.1f}".format(np.mean(l), math.exp(np.mean(l))))
        if((ty =="test") and not config.emo_beta):
            return_dict = model.decoder_greedy(batch, max_dec_step=30, save=config.save)
            sent_g = return_dict['sent']
            if config.save:
                bow_res.append([return_dict['bow']] + [return_dict['bow_{}'.format(i)] for i in range(10)])

            for i, greedy_sent in enumerate(sent_g):
                rf = " ".join(batch["target_txt"][i])
                hyp_g.append(greedy_sent)
                ref.append(rf)
                # show = 1 if ty == 'valid' else 70
                # if j < show and i < 3 and j == 61:
                #     #print('[BOW]', boww[i])
                #     print('[ATOMIC_T]', " ".join(batch['atomic_t_text'][i]))
                #     print('[ATOMIC]', " ".join(batch['atomic_text'][i]))
                #     # print(batch['atomic_know_t'])
                #     print('[CTX]:', " ".join([w for s in batch['input_txt'][i] for w in s]))
                #     print('[REF]:', rf)
                #     print('[GEN]:', greedy_sent)
        else:
            continue
    import numpy as np
    ...
    all_batch_npz = np.concatenate(all_batch_npz, 0) # n, 10, dim
    for t in range(10):
        s_batch_npz = all_batch_npz[:,t,:]
        bs = s_batch_npz.shape[0]
        X_tsne = tsne.fit_transform(s_batch_npz)
        x_min, x_max = X_tsne.min(0), X_tsne.max(0)
        X_norm = (X_tsne - x_min) / (x_max - x_min)  # 归一化
        plt.figure(figsize=(8, 8))
        y = []
        for i in range(bs):
            y.extend([t])  # 0,1,2,3; 0, 10
        for i in range(X_norm.shape[0]):  # bs, 10
            plt.text(X_norm[i, 0], X_norm[i, 1], str(y[i]), color=plt.cm.Set1(y[i]),
                     fontdict={'weight': 'bold', 'size': 9})
        plt.xticks([])
        plt.yticks([])
        plt.savefig('pic{}_{}.png'.format(0, t))
    bs = all_batch_npz.shape[0] #n
    all_batch_npz = all_batch_npz.reshape(-1, 300)  #n*10, 300

    X_tsne = tsne.fit_transform(all_batch_npz)
    x_min, x_max = X_tsne.min(0), X_tsne.max(0)
    X_norm = (X_tsne - x_min) / (x_max - x_min)  # 归一化
    plt.figure(figsize=(8, 8))
    y = []
    for i in range(bs):
        y.extend([j for j in range(10)]) #0,1,2,3; 0, 10
    for i in range(X_norm.shape[0]): #bs, 10
        plt.text(X_norm[i, 0], X_norm[i, 1], str(y[i]), color=plt.cm.Set1(y[i]),
                 fontdict={'weight': 'bold', 'size': 9})
    plt.xticks([])
    plt.yticks([])
    plt.savefig('pic{}.png'.format(0))
    # print(trans.keys())
    # k_p = [np.mean(h) for h in k_p]
    # print('k_p', k_p)
    # k_r = [np.mean(h) for h in k_r]
    # print('k_r', k_r)
    # print('k_p', np.mean(k_p))
    # print('k_r', np.mean(k_r))
    #
    # print(trans.keys())
    # k_p_d = [np.mean(h) for h in k_p_d]
    # print('k_p_d', k_p)
    # k_r_d = [np.mean(h) for h in k_r_d]
    # print('k_r_d', k_r_d)
    # print('k_p_d', np.mean(k_p_d))
    # print('k_r_d', np.mean(k_r_d))

    print('diverse', len(set(post_labels)) / len(post_labels))
    ...
    if config.save:
        pkl.dump(bow_res, open('bow_res.pkl', 'wb'))
    print("EVAL\tLoss\tPPL\tVAE\telbo\td1\td2\td3\temo_loss\tacc\talpha")
    print("{}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.2f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}".format(ty,loss,math.exp(loss), dis, elbo, gd1,gd2,gd3, el, acc, alpha))
    this_res = {}
    if len(ref) != 0:
        pkl.dump({'ref': ref,
                  'hyp_g': hyp_g,
                  ...
                  'acc': acc,
                  'bow': bow,
                  }, open(res_store_file, 'wb'))
        er, ip, ex, bleu, rouge, mi_dist1, mi_dist2, avg_len = count_new_metrics(res_store_file, dataset=config.dataset)
        cls_emo(res_store_file)
        this_res = {
            'gd1': mi_dist1,
            'gd2': mi_dist2,
            'ppl': math.exp(loss),
    ...
    return elbo, math.exp(loss), dis, bow, elbo, bleu_score_g, gd1,gd2,gd3, el, this_res
def evaluate_v(model, data,  ty='valid', max_dec_step=50,  do_label=False):
    model.eval()
   

    res_store_file = config.test_name + '.' + 'res.pkl'
    ...
    for j, batch in pbar:
        return_dict = model.train_one_batch(batch, 0, train=False, valid_ppl=True)
        loss = return_dict['loss_rec']
        ppl = return_dict['ppl']
        dis_loss = return_dict['dis_loss']
        bow_prog = return_dict['aux']
        ...
        elbo.append(elbo_prog)
        pbar.set_description("loss:{:.4f} ppl:{:.1f}".format(np.mean(l), math.exp(np.mean(l))))
        if(ty =="test" or (ty =="valid" and j< 20) or config.pse):
            return_dict = model.decoder_greedy(batch,max_dec_step=30)
            sent_g = return_dict['sent']
            for i, greedy_sent in enumerate(sent_g):
                rf = " ".join(batch["target_txt"][i])
                hyp_g.append(greedy_sent)
    ...
    return elbo, math.exp(loss), dis, bow, elbo, bleu_score_g, gd1, gd2, gd3


def evaluate_cls(model, data, ty='valid', max_dec_step=50, do_label=False):
    res_store_file = 'use_atomic {} sep {} bs {} layer {}'.format(config.use_atomic, config.sep_atomic,
                                                                  config.batch_size, config.hop)
    post_label_file = config.test_name + ty + 'pri.pkl'
    model.__id__logger = 0
    pri_labels = []
    if ty == "test":
        print("testing generation:")
    ...
    for j, batch in pbar:
        return_dict = model.train_one_batch(batch, 0, train=False)
        loss = return_dict['loss_vae_cls']
        vacc = return_dict['vae_acc']
        c_r_acc = return_dict['c_r_acc']
        pri_label = return_dict['encoding']
        batch_vq_label = pri_label.tolist()#.detach().cpu().numpy().tolist()
        pri_labels.extend(batch_vq_label)
        l.append(loss)
        acc.append(vacc)
        c_r_accs.append(c_r_acc)
    loss = np.mean(l)
    c_r_acc = np.mean(c_r_accs)
    acc = np.mean(acc)
    if do_label:
        with open(post_label_file, 'wb') as f:
            pkl.dump(pri_labels, f)
    print("EVAL\tLoss\tacc\tc_r_acc")
    print("{}\t{:.4f}\t{:.4f}\t{:.4f}".format(ty, loss, acc, c_r_acc))
    return acc
...
def gaussian_kld(recog_mu, recog_logvar, prior_mu, prior_logvar):
    kld = -0.5 * torch.sum(1 + (recog_logvar - prior_logvar) - torch.div(torch.pow(prior_mu - recog_mu, 2), torch.exp(prior_logvar))
